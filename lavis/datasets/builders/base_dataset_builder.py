...
class BaseDatasetBuilder:
    ...
    def build_processors(self):
        ...
        if pts_proc_cfg is not None:
            ...
            self.pts_processors["train"] = self._build_proc_from_cfg(pts_train_cfg)
            self.pts_processors["eval"] = self._build_proc_from_cfg(pts_eval_cfg)
    ...
    def _download_pts(self):
        ...
        for i in range(len(self.data_type)):  
            storage_path = self.config.build_info.get(self.data_type[i]).storage
    ...
    def build(self):
        ...
        for split in ann_info.keys():
            ...
            pts_processor = (
                self.pts_processors["train"]
                if is_train
                else self.pts_processors["eval"]
            )
