...
def handle_found_object(
    ...
    # temp_dir = '/nvme/datasets/objaversexl_render/tmp'                                                  # temp_dir
    ...
    if platform.system() == "Linux" and using_gpu:
        args += " --engine CYCLES"                      # " --engine BLENDER_EEVEE" can not use 
    ...
    if (
        (len(png_files) != num_renders*3)
        # or (len(npy_files) != num_renders)
        or (len(metadata_files) != num_renders+2)
    ):
        logger.error(
    ...
    os.remove(f"{target_directory}.zip")
    return True
...
def render_objects(
    json_file: str = "./annotations/first5w_500_per_file/first5w_500_per_file_1.json",                                       # annotation file
    render_dir: str = "./render_dir",                                                                 # render_dir
    temp_dir: str = './temp_dir',                                                                     # Add this argument
    ...
    logger.info(f"Using {processes} processes to render.")
    # get the objects to render
    objects = get_example_objects(json_file=json_file)
