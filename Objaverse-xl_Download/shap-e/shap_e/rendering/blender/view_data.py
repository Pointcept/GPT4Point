...
class BlenderViewData(ViewData):
    ...
    def __init__(self, f_obj: BinaryIO, uid):
        self.zipfile = zipfile.ZipFile(f_obj, mode="r")
        self.infos = []
        self.uid = uid
        with self.zipfile.open(f"{self.uid}/info.json", "r") as f:
        ...
        for i in range(20):
            name = f"{i:05}.json"  #!注意这里要改成i:03, 额 不用
            # if name not in names:
            #     break
            with self.zipfile.open(f"{self.uid}/{name}", "r") as f:
    ...
    def load_view(self, index: int, channels: List[str]) -> Tuple[Camera, np.ndarray]:
        ...
        if any(x in channels for x in "RGBA"):
            with self.zipfile.open(f"{self.uid}/{index:03}.png", "r") as f: #!这里需要改一下 03
                rgba = np.array(Image.open(f)).astype(np.float32) / 255.0
                channel_map.update(zip("RGBA", rgba.transpose([2, 0, 1])))
        if "D" in channels:
            with self.zipfile.open(f"{self.uid}/{index:03}_depth.png", "r") as f:
        ...
        if "MatAlpha" in channels:
            with self.zipfile.open(f"{self.uid}/{index:03}_MatAlpha.png", "r") as f:
