...
def render_views_from_stf(
    ...
    *,
    ...
    # Tiange: remove mask-related things as we only do inference
    ...
    #Tiange
    out.channels = out.channels * channel_scale.view(-1)
...
def _render_with_pytorch3d(
    ...
    with torch.autocast(device_type, enabled=False):
        ...
        # Tiange modified here
        #alpha = imgs[..., -1:]
        #mask = (alpha < 0.000001).expand_as(imgs)   
        #imgs[mask] = 222.0/255.0  #
        out = AttrDict(
            ...
            # transmittance= views[..., -1:],  # [batch_size, *inner_shape, height, width, 1]
            # transmittance= torch.ones(views[...,-1:].shape).cuda(),  # [batch_size, *inner_shape, height, width, 1]
            meshes=meshes,
        )
